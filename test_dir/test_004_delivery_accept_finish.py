    ...
    if boss_delivery_order_url == browser.current_url:
        ...
        if accept_flag:
            ...
            if len(finish_ords) >= 1:
                ...
                log.warn('当前门店：{} 无 待完成的订单'.format(store_no))
...
def get_store_wait_accept_ords(store_no: str, biz_state: str):
    ...
    ords = mysql.select(
                 ...
                 "AND `biz_state` = '{biz_state}' ORDER BY `update_time` DESC "
                 .format(store_no=store_no, biz_state=biz_state))
    ords_list = []
    if isinstance(ords, list):
        for i in range(len(ords)):
            ords_list.append(ords[i]['order_no'])
    else:
        log.warn('门店：{store_no},当前无biz_state：{biz_state} 订单'.format(store_no=store_no, biz_state=biz_state))
    return ords_list
